...
- For the auth proxy server, set `AUTH_REDIRECT_PROXY_URL` to the same as above, as well as `GITHUB_CLIENT_ID`, `GITHUB_CLIENT_SECRET` (or the equivalent for your OAuth provider(s)). Lastly, set `AUTH_SECRET` **to the same value as in the Next.js app** for preview environments.
